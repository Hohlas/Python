import urllib, http.client    # https://bablofil.ru/birja-excel/
import os
import time
import json
...
ws = wb.active
# Вставим заголовки
ws.append(["pair", "byu", "sell", "last_trade", "Кол-во по сделке", "Цена сделки", "Сумма сделки"])

list = call_api(method='ticker', pair=pairs_str, limit=10000)
for pair in list: # пройдемся по каждой паре
    b=list[pair]
    if not list[pair]: #пропускаем пары, по которым не было торгов
        continue
    c=b['high']
    k=list[pair]['high']
    # Теперь проходим по всем торгам этой пары
    for prm in list[pair]:
        a=list[pair][prm]
